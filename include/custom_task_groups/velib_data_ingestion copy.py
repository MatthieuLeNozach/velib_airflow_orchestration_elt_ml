        ...
        **kwargs
        ...
        def check_data_integrity(processed_data):
            ...
            if self.table_type == "locations":
                ...
                if row_count >= 1460:
                    logging.info("Row count check passed. Total rows: %d", row_count)
                else:
                ...
                if result:
                    logging.error("Timestamp %s already exists in the database. Failing the task.", latest_timestamp)
                    raise ValueError(f"Timestamp {latest_timestamp} already exists in the database.")
                else:
                    logging.info("Timestamp %s does not exist in the database. Proceeding with data insertion.", latest_timestamp)
